...
function buscarPelicula(termino) {
    const resultados = peliculas.filter(p => p.titulo.toLowerCase().includes(termino.toLowerCase()));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    filtroGenero === null || filtroGenero === void 0 ? void 0 : filtroGenero.addEventListener('change', (e) => {
        const target = e.target;
        filtrarPorGenero(target.value);
    });
    const buscador = document.getElementById('buscador');
    buscador === null || buscador === void 0 ? void 0 : buscador.addEventListener('input', (e) => {
        const target = e.target;
